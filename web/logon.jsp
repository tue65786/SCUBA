...
req+=Boolean.toString(isMobile);
            String user_Name = request.getParameter("uname");
            String usrNameParsed = "";
            String user_Role = "";
            String pwEncrypted = "";
            ...
            if(user_Name == null) {
                ...
                if(user_Role.length() > 0) {
                    int locSym = user_Name.indexOf('@');
                    if(locSym > 0) {
                        usrNameParsed = user_Name.substring(0, locSym);
                        usrNameParsed = usrNameParsed.replace('.', ' ');

                    }
                    else {
                        usrNameParsed = user_Name;
                    }
                    session.setAttribute("userName", usrNameParsed);
                    session.setAttribute("userRole", user_Role);
                    session.setAttribute("userid",(String) userData.webUserId );
                    
                    String welcomeMsg = "Welcome back <i>" + usrNameParsed + "</i> to the SCUBA Dive Log web site!";
                    response.sendRedirect("index.jsp?wm=" + welcomeMsg);
                } // successful log in
